...
export class AudioPlayerComponent implements OnInit, OnChanges {
  ...
  toggleRepeat() {
    ...
    console.log('track.lenth == ', this.tracks.length);
    if (this.repeat === 'none') {
      this.repeat = 'all';
  ...
  ngOnInit() {
    ...
    this.audioPlayerService.getPlaylist().subscribe(tracks => {
      if (tracks !== null && tracks !== []) {
        this.tracks = tracks;
        console.log('track.length tai init ', this.tracks.length);
        this.initialize();
        this.play()
      }
    });
  ...
  previousSong(): void {
    ...
    if (!this.checkIfSongHasStartedSinceAtleastTwoSeconds()) {
      ...
      console.log('xuong else resetsong');
      this.resetSong();
    }
    this.updateCurrentTrack();
    console.log('truoc goi play1 ()');
    this.play();
  }

  resetSong(): void {
    console.log('reset Play List');
